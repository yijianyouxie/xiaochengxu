...
module.exports = class TaskController extends Controller {
	...
	async signin() {
		...
		if (log.getScore) {
			...
			let {score} = await this.service.user.user.editScore(uid, bounty, `${notice}`, 0, {
				task: log
			});
			//系统每日积分总计
	...
	async share() {
		...
		let log = await this.service.user.task.save(uid, task_id);
		if (log.getScore) {
			let {score} = await this.service.user.user.editScore(uid, log.bounty, `${log.task.name}`, 0, {
				task: log
			});
			await this.service.system.app.setScoreDayStatistics(score);
		}
	...
	async getdouble() {
		...
		if (task) {
			//更新用户积分
			let {score} = await this.service.user.user.editScore(uid, task.bounty, notice, 0, {
				task: log
			});
			await this.service.system.app.setScoreDayStatistics(score);
			//更新子模块以获取双倍积分状态
	...
	async finish() {
		...
		let log = await this.service.user.task.save(uid, task_id);
		if (log.task.bounty == 0) {
			log.getScore = log.result = false
		}
		if (log.getScore) {
			let {score} = await this.service.user.user.editScore(uid, log.task.bounty, `${log.task.name}`, 0, {
				task: log
			});
			await this.service.system.app.setScoreDayStatistics(score);
		}
