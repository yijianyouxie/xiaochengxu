...
module.exports = class MallService extends Service {
	constructor(ctx) {
		super(ctx)
		this.collection = this.db.collection("tian-payment-qrcodes")
	...
	async info(id, userInfo, data) {
		...
		if (shop_id) {
			...
			shop_id = id;
			id = await this.service.user.user.genIdentityId("tian-payment-qrcodes", 1, 1000000)
		}
		//查询店铺信息
		...
		return {
			...
			profit_sharing: info.shop.settlement_mode == "profit_sharing", //订单是否支持分账
			state: 0,
			id: id,
	...
	async afterPayment(payinfo, order) {
		...
		console.log("工单订单支付完成回调成功，更新订单信息")
