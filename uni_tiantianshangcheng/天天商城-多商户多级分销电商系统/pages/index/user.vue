			...
			<view class="header-panel padding-bottom-22 ">
				...
				<view class=" margin-left-30 colorWhite flex-1">
					...
					<view class="font-24 padding-top-14" v-if="userInfo.my_invite_code">
						...
						<text class="font-20 margin-left-6 bg-white colorRed padding-lr-10"
							v-if="!userInfo.inviter_uid || userInfo.inviter_uid.length==0"
							@click="checkTeam()">未生效</text>
					</view>
		...
		<view class="content-body margin-22 padding-22 bg-white border-radius" v-if="adminShops && adminShops.length>0">
			...
			<view class="">
				<view v-for="(item,index) in adminShops" :key="index"
					...
					<image :src="item.src" mode="aspectFill" class="image-width-80 border-radius"></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			navToUserInfo() {
				//#ifdef MP-WEIXIN
				if (!this.userInfo.hasNickName && wx.getUserProfile) {
					//没有授权，必须获取用户信息，新接口，需要判断一下
					wx.getUserProfile({
