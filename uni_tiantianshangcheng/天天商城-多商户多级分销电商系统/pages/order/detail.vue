<template>
	<view>
		...
		<view class="item address ti-flex" v-if="item.address">
			<tian-icons type="shouhuodizhi1" align="right" color="#eb4e38" size="30"> </tian-icons>
			<view class="flex-1 margin-left-6">
				<view class="" @click="tel(item.address.mobile)">
					...
					<tian-icons type="dianhua" color="#eb4e38" class="margin-left-10"></tian-icons>
				</view>
				<view class="font-20 margin-top-12 colorGray">
		...
		<view class="item">
			...
			<view class="margin-top-12" v-if="item.invoice">
				<text>发票抬头：</text>
		...
		<block v-if="item.delivers && item.delivers.length > 1">
			<view class="font-24 ti-flex between padding-24 tabs">
				<view v-for="(deli,index2) in item.delivers" :key="index2" @click="showExpress(deli,index2)"
					:class="{current:expressIndex==index2}">
	...
	export default {
		...
		methods: {
			...
			showExpress(item, index) {
				...
				this.expressGoods = this.item.goods.filter(e => e.delivery && e.delivery.number == item
					.number);
